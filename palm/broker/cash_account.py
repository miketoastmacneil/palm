...
class TransactionRecord:
    ...
    def __init__(self, transaction_type, transaction_response, previous_balance, current_balance):
...
class  CashAccount:
    ...
    def __init__(self, initial_deposit):
        ...
        self._transaction_history = [TransactionRecord(Transaction.DEPOSIT, 
    ...
    def _handle_negative_withdrawal_request(self):
        response = WithdrawalResponse(WithdrawalResult.DECLINED, WithdrawalResponse.DeclinedReason.NEGATIVE_AMOUNT_REQUESTED)
        previous_balance = self.balance
        new_balance = previous_balance
        record = TransactionRecord(Transaction.WITHDRAWAL,
                                    response, 
                                    previous_balance,
                                    new_balance)
    ...
    def _handle_insufficient_funds_withdrawal_request(self):
        response = WithdrawalResponse(WithdrawalResult.DECLINED, WithdrawalResponse.DeclinedReason.INSUFFICIENT_FUNDS)
        previous_balance = self.balance
        new_balance = previous_balance
        record = TransactionRecord(Transaction.WITHDRAWAL,
                                    response, 
    ...
    def _handle_negative_deposit_request(self):
        response = DepositResponse(DepositResult.DECLINED, DepositResponse.DeclinedReason.NEGATIVE_AMOUNT_DEPOSITED)
        previous_balance = self.balance
        new_balance = previous_balance
        record = TransactionRecord(Transaction.DEPOSIT,
                                    response, 
                                    previous_balance,
                                    new_balance)
        self._transaction_history.append(record)    
