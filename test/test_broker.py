...
@pytest.fixture
def context(eod_data: pm.EquityEOD):
...
@pytest.fixture
def buy_order():
    return pm.MarketOrder.Buy("AAPL", 10)

@pytest.fixture
def sell_order():
    return pm.MarketOrder.Sell("AAPL", 10)

@pytest.fixture
def broker(context, initial_deposit):
    return pm.SimulatedBroker(context, initial_deposit)
...
def test_SufficientBalanceBuyOrderSubmitted_OrderFulFilled(broker, buy_order):
    ...
    assert buy_order.status == pm.MarketOrderStatus.CLOSED
...
def test_NewSellOrderSubmitted_OrderFulfilled(broker, sell_order):
    ...
    assert sell_order.status == pm.MarketOrderStatus.CLOSED
...
def test_TwoBuyOrdersSameQuantitySubmitted_LongPositionDoubled(broker, buy_order):
    ...
    broker.submit_order(buy_order)
    new_buy = pm.MarketOrder.Buy(buy_order.symbol, buy_order.quantity)
...
def test_TwoSellOrdersSameQuantitySubmitted_ShortPositionDoubled(broker, sell_order):
    ...
    broker.submit_order(sell_order)
    position = broker.get_position(sell_order.symbol)
    new_sell = pm.MarketOrder.Sell(sell_order.symbol, sell_order.quantity)
...
def test_BuyOrderAfterSellOrderSameQuantity_ShortPositionClosed(broker, sell_order):
    ...
    new_buy = pm.MarketOrder.Buy(sell_order.symbol, sell_order.quantity)
    broker.submit_order(new_buy)
    assert position.quantity == 0
    assert position.status == pm.Position.Status.CLOSED
